<?php 
	...
	$deckData = new deck();
		...
		if (!isset($_SESSION['username'])) {
			header("location: ".$main."login.php?re=/my/favorites");
		}
		
		if (isset($_POST['delFavDeck']) && !empty($_POST['deckID'])) {
			$deckData->removeFavoriteDeck($_SESSION['username'], $_POST['deckID']);
		}

		$query = $db->prepare("
			SELECT * FROM decks
			INNER JOIN favDeck
			ON favDeck.deck_id = decks.id WHERE favDeck.user = :user");
		$arr = array(
			...
			<div class="decks div-margin">
				...
				<table>
					...
					<tr>
						...
						<td><a href="<?php echo($main) ?>deck/<?php echo($deck['deck_id']) ?>"><?php echo($deck['deck_title']) ?></a></td>
						<td>
						...
						<td><?php echo($x->totalComments($deck['deck_id'], 2)) ?></td>
						<td><?php echo($x->ago($deck['time'])) ?></td>
						<td>
							<form method="post" action="">
								<input type="hidden" name="deckID" value="<?php echo($deck['deck_id']) ?>" />
								<button type="submit" name="delFavDeck" class="btn-modern">Delete</button>
							</form>
						</td>
					</tr>
				<?php } ?>
