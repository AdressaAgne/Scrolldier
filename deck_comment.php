		...
		<div class="container">
				...
				<div class="clearfix" id="ScrollsNr<?php echo($listOfScrolls[$j][3]); ?>">
					<div id="" class="deckScrollList <?php echo($scrollClass) ?> " style="overflow: hidden;">
						...
						<span class="right">
							<a href="resources/cardImages/<?php echo($listOfScrolls[$j][3]) ?>.png" data-title="<?php echo($listOfScrolls[$j][2]); ?>, x<?php echo($listOfScrolls[$j][4]); ?>" data-lightbox="Scrolls"><img class="listScroll" src="resources/cardImages/<?php echo($listOfScrolls[$j][3]) ?>.png" alt="" /></a>
						</span>
				...
				<div class="last clearfix">
					...
					<?php if ($row['deck_author'] == $_SESSION['username']) { ?>
						<a href="?d=<?php echo($row['id']) ?>&edit" class="btn-modern left">edit (comming soon)</a>
					<?php } ?>
							</div>
				...
				<?php } ?>
					...
					<div class="left div-half div-margin">
						<div class="div-1">
							<label>Author: <a href="u/?u=<?php echo($row['deck_author']) ?>"><?php echo($row['deck_author']) ?></a></label>
						</div>
						
						<div class="div-1">
							<label>Deck Name: <a href="http://www.scrollsguide.com/deckbuilder/#<?php echo($row['link']) ?>" target="_blank"><?php echo($row['deck_title']) ?></a></label>
						</div>
						<div class="div-1">
					...
					<div class="right div-half div-margin">
						<div class="div-1">
							<label>Version: <?php echo($row['meta']) ?></label>
						</div>
						<div class="div-1">
							<label>Scrolls: <?php echo($row['scrolls']) ?></label>
						</div>
						<div class="div-1">
							<label>Score: <?php echo($row['vote']) ?></label>
						</div>
						<div class="div-1">
							<label>Competitive: <?php if ($row['competative'] == 1) {
								echo("Yes");
							} else {
								echo("No");
							}
						...
						<div class="div-1">
							<label>Submitted <?php echo($x->ago($row['time'])) ?></label>
						</div>
					</div>
					
					<div class="div-3">
						<p><?php echo(makeClickableLinks(strip_tags($row['text']))) ?></p>
					</div>
				</div>
				...
				?>
					...
					<div class="avatar scrolls">
						<img src="resources/head_<?php echo($comment['headID']) ?>.png" alt="" />
					</div>
					<div class="commentPost scrolls">
						<h4 class="clearfix"><a class="left" href="u/index.php?u=<?php echo($comment['byUser']) ?>"><?php echo(strip_tags($comment['byUser'])) ?></a>
